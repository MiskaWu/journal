...
| **O5** | aggregator 指定哪台? | **2026-07-31 定案:留在個人 dev 機**。原設計屬意 infra 機(24/7),但納管時使用者釐清:**infra 是公司資產,工作與資料是個人的** —— 聚合權責與 progress 的擁有權應留在個人機器;infra 只當回報 node。未來遷往個人的可靠環境時,一行 `journal init --role aggregator --force-takeover` 接手。同時只能有一台。 |
...
| **O10** | **ssh alias 要不要改成寫 `~/.ssh/config.d/` drop-in?** | **要,且 drop-in 是預設落點**(2026-08-06 實作,`jr_ssh_alias`)。**緣由**:`~/.ssh/config` 已有三方在寫 —— wsl-bootstrap 的 managed 區塊、dev-env-ansible `make ssh-host` 補的 `Include`、journal 檔尾 append 的 alias 段;三方拿 grep/sed 切同一個檔,正是 drop-in 要消掉的形狀。改過去之後 journal 只擁有 `~/.ssh/config.d/journal.github.com.conf` 一個檔。<br><br>**實測推翻了兩個原本的假設**:<br>① **`Include` 只能插檔首**。它出現在 `Host` 區塊之後就屬於該區塊,變成*條件式*包含 —— 只有該 host 匹配時才處理,看起來像沒寫。所以「append 一行 Include」不成立,代改主檔就是重寫別人的檔,因此**一律先問過**(`jr_ssh_ask_include`,講明改哪一行、會備份、驗不過自動還原),非互動路徑一律不動主檔。<br>② **老 ssh 讀到 `Include` 不是降級是全滅**:`terminating, bad configuration options`、rc=255,那個使用者**所有的** ssh 都掛,不只 journal。<br><br>**降級改用功能探測,不比對版本號**:寫完跑 `ssh -G` 看解析得到不,不通過就原樣還原、退回舊做法(append 主檔)。`ssh -G` 是 6.8+、`Include` 是 7.3+,凡是支援 Include 的必定支援 `-G`,探測不會誤殺 —— 而 `ssh -V` 拿 `10.2` 跟 `7.3` 比字串大小還會反過來。這比版本閘更貼 **D10**。<br><br>**兩個坑記著**:`HostName github.com` 是命脈(`journal.github.com` 無 DNS 紀錄,純 alias);ssh 讀*預設* config 走 getpwuid **不認 `$HOME`**,測試的假 HOME 會被穿透,所以探測明寫 `-F`、測試裡的 `Include` 一律絕對路徑。<br><br>**附帶補掉一個既存落差**:檔案裡的註解一直寫「`uninstall --purge` 移除」,但 purge 從來沒刪過那段(只印「留給你自己收」)。drop-in 讓移除變成刪一個檔,主檔殘留也有結構式移除可用(照 ssh_config 文法,`Host` 區塊終止於下一個頂層指令,不靠猜空行),話終於兌現得了。既有安裝在互動下會被問要不要搬家。 |
